...
local function ConcatModID(str)
	if this.private.ConcatCache[str] == nil then
        ...
        this.private.ConcatCache[str] = result
        return result
    else
...
function UI.BetterTooltipsNoCenter(callback, z, xOffset, yOffset, leftMargin, rightMargin, always, nx, ny, IsDrawAni, displayAutoBox)
	local left_click, right_click, hover, x, y, width, height, draw_x, draw_y, draw_width, draw_height =
        GuiGetPreviousWidgetInfo(this.public.gui)
...
---@param mw number
---@param mh number
function UI.InputBlock(id, x, y, w, h, mw, mh)
...
---@param id string
---@param info GuiInfo
function UI.InputBlockEasy(id, info)
	UI.InputBlock(id, info.x - 2, info.y - 2, info.width, info.height, 2, 2)
end

---@return boolean
---@return integer
---@return integer
function UI.Drag(srcx,srcy)
    local info = UI.WidgetInfoTable()
    UI.InputBlockEasy("__Drag__Block", info)
    if not info.hovered and this.private.DragX == nil then
        return false, srcx, srcy
    end
    if not InputIsMouseButtonDown(Mouse_left) then
        this.private.DragX = nil
        this.private.DragY = nil
        this.private.DragZeroX = nil
        this.private.DragZeroY = nil
        return false, srcx, srcy
    end
    ...
    if this.private.DragX == nil then
        ...
        this.private.DragZeroY = my
    end
    local x = this.private.DragX
    local y = this.private.DragY
    local zeroX = this.private.DragZeroX
    local zeroY = this.private.DragZeroY
    if math.floor(mx) == math.floor(zeroX) and math.floor(my) == math.floor(zeroY) then
        return false, srcx, srcy
    end
    return true, mx - zeroX + x,my - zeroY + y
end
